...
-- Migration: 010_fix_date_trunc_timezone.sql
-- Description: Fix date_trunc calls that incorrectly use 'Monday' as timezone
-- Date: March 2026

-- =============================================
-- FIX DATE_TRUNC CALLS
-- =============================================
...
BEGIN
    ...
    -- Get current week start (Monday) - fixed: use date arithmetic instead of date_trunc with timezone
    week_start_date := (CURRENT_DATE - EXTRACT(DOW FROM CURRENT_DATE - 1)::INTEGER * INTERVAL '1 day')::DATE;
...
BEGIN
    -- Get week start (Monday) - fixed: use date arithmetic
    week_start_date := (CURRENT_DATE - EXTRACT(DOW FROM CURRENT_DATE - 1)::INTEGER * INTERVAL '1 day')::DATE;
