...
-- Migration: 011_force_fix_date_trunc.sql
-- Description: Force fix date_trunc by dropping and recreating functions
-- Date: March 2026

-- =============================================
-- FORCE FIX DATE_TRUNC CALLS
-- =============================================

-- Drop functions first to force recreation
DROP FUNCTION IF EXISTS check_weekly_submission_limit(UUID);
DROP FUNCTION IF EXISTS get_or_create_weekly_limit(UUID);

-- Recreate check_weekly_submission_limit with fixed date arithmetic
...
BEGIN
    ...
    -- Get current week start (Monday) using date arithmetic
    week_start_date := (CURRENT_DATE - EXTRACT(DOW FROM CURRENT_DATE - 1)::INTEGER)::DATE;
...
-- Recreate get_or_create_weekly_limit
...
BEGIN
    -- Get week start (Monday) using date arithmetic
    week_start_date := (CURRENT_DATE - EXTRACT(DOW FROM CURRENT_DATE - 1)::INTEGER)::DATE;
